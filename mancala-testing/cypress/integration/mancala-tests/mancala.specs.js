...
// })
  ...
  it('views a complete game and an incomplete game in index', () => {
    ...
    // Checks game data
    cy.get('.index').children().should('have.length', 2)
    
    //// NEEDS FIXING: FIGURE OUT HOW TO SELECT INDIVIDUAL CHILDREN IN DIV
    // cy.get('.index>a').eq(0).click()
    // cy.get('.index' nth-child(0)]'').contains('GAME OVER')
  })
})
